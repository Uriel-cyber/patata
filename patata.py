...
# 1. Sliders para la Función Objetivo (Cambian de 1 en 1)
mbps_celular = st.sidebar.slider("Eficiencia Celular (Mbps por Vatio)", min_value=5, max_value=30, value=15, step=1)
mbps_wifi = st.sidebar.slider("Eficiencia Wi-Fi (Mbps por Vatio)", min_value=20, max_value=60, value=40, step=1)

st.sidebar.header("⚠️ Restricciones del Sistema")

# 2. Sliders para las Restricciones Generales (Todos con step=1)
max_pot_dron = st.sidebar.slider("Potencia máxima por Dron (W)", min_value=5, max_value=20, value=12, step=1)
max_pot_enjambre = st.sidebar.slider("Presupuesto Total Enjambre (W)", min_value=20, max_value=80, value=40, step=1)
max_backhaul = st.sidebar.slider("Límite Backhaul Satelital Wi-Fi (Mbps)", min_value=500, max_value=2000, value=1200, step=1)

col1, col2 = st.columns([1, 2])

with col1:
    st.subheader("📋 Restricciones Geográficas / Interferencia")
    # Cambiados a sliders que aumentan de 1 en 1 estrictamente
    max_interf = st.slider("Máxima potencia Celular D1 + D2 (Evitar Interferencia)", min_value=1, max_value=20, value=10, step=1)
    min_bosque = st.slider("Mínima potencia Celular D3 + D4 (Penetración Bosque)", min_value=1, max_value=20, value=8, step=1)
...
with col2:
    ...
    if boton_optimizar:
        ...
        if res.success:
            ...
            # Formatear resultados en columnas
        ...
        st.warning("👈 Configura las restricciones en el panel izquierdo y haz clic en 'Calcular Optimización' para ver los resultados.")
